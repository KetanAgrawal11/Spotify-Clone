...
const getSongUrl = async () => {
    ...
    let a = await fetch(currentPageUrl + "/songs/mp3%20files/");
    ...
    songs.forEach((song, index) => {
        ...
        <div class="songInfo text-white">
            <div class="name text-sm">${removeChar(song.slice(song.indexOf("les/")+4, -4), '%20')}</div>
            <div class="artistName text-xs">Ketan Agrawal</div>
        </div>
    </div>`
        songNameList.push(removeChar(song.slice(song.indexOf("les/")+4, -4), '%20'))
        let title = removeChar(song.slice(46, -4), '%20')
        let div = document.createElement("div")
    ...
    response = await playlistFloders.text()
